...
CREATE TABLE IF NOT EXISTS citations (
    omid VARCHAR({params.omid_max_len}) PRIMARY KEY, 
    citations INT({params.citations_max_len}),
...
for chunk in pd.read_csv("dblp/citations.csv", chunksize=chunk_size):
    ...
    if stop_on_db_size and (
        citations_updates_counter >= db_cit_size
        or citations_line_reads_counter >= lines_read_limit_cit
    ):
        break
...
for chunk in pd.read_csv("dblp/omid.csv", chunksize=chunk_size):
    ...
    if stop_on_db_size and (
        omid_updates_counter >= db_omid_size
        or omid_line_reads_counter >= lines_read_limit_omid
    ):
        break
